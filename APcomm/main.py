...
def sendHello(stop_event):
    ...
    while True:
        ...
        msg = SerialMessage(
            ...
            connectedNodes[0],
            5,
            struct.pack("<BB",0,PINSTATE[0])
        )
        i+=1
        sendQueue.put(msg)
        time.sleep(0.2)
        PINSTATE[0] = not PINSTATE[0]
...
def sendCallback(ch, method, properties, body):
...
channel.basic_consume(queue='sendQueue',
                      on_message_callback=sendCallback,
                      auto_ack=False)
